...
- **Remote server (LM Studio & Ollama)** - point the app at an OpenAI-compatible server on your LAN, or **scan** the network to find one automatically. Servers appear as an entry in the model picker, where the available models are **grouped by provider** (Qwen, Gemma, …) and listed by their cleaned-up names. Remote chat streams with reasoning, accurate tokens/second, and a server-side **offload** when you unload. A **model-details card** (frosted, matching the picker) shows the server model's real quantization, context window, and live generation-parameter sliders. Away from home, point it at the server's [Tailscale](https://tailscale.com/) IP for the same thing LM Studio's "LM Link" does.
- **Project folders** - organize chats into collapsible folders in the sessions drawer; deleting a folder also deletes the chats inside it.
