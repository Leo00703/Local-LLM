...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * File attachments on a message. Single attachment per message (v1);
         * attachmentKind routes ("document" → attachmentText; "image" → the
         * future vision pixels at attachmentUri). Existing rows get NULL/0 and
         * stay untouched, so no conversation is lost — and vision reuses these
         * columns with no further migration.
         */
        private val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE chat_messages ADD COLUMN attachmentName TEXT")
                db.execSQL("ALTER TABLE chat_messages ADD COLUMN attachmentMime TEXT")
                db.execSQL("ALTER TABLE chat_messages ADD COLUMN attachmentKind TEXT")
                db.execSQL("ALTER TABLE chat_messages ADD COLUMN attachmentText TEXT")
                db.execSQL("ALTER TABLE chat_messages ADD COLUMN attachmentUri TEXT")
                db.execSQL("ALTER TABLE chat_messages ADD COLUMN attachmentTruncated INTEGER NOT NULL DEFAULT 0")
            }
        }

        /**
         * Multiple attachments per message: a JSON array column. The v9 single
         * columns stay (read-only legacy); new rows write attachmentsJson.
         */
